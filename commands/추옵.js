...
const weapon=["블레이드","아대","건","에너지소드","소울슈터","너클","건틀렛리볼버","폴암","활","에인션트보우","듀얼보우건","단검","체인","부채","한손검","한손둔기","한손도끼","케인","석궁","창","두손검","두손둔기","두손도끼","데스페라도","핸드캐논","완드","샤이닝로드","ESP리미터","매직건틀렛","스태프","튜너"];
...
function printmorezero(client, msg, args,titlen,print)
{
	var arr=print.split("\n\n");
...
function printmore(client, msg, args,titlen,print)
{
	var arr=print.split("\n\n");
	let helpMenu = new Menu(msg.channel, msg.author.id, [
            {
...
function printchuop(client, msg, args)
 {
	 var series=args[1];
	 if(series=="류드")
	 {
		 msg.channel.send("류드의 검\n기본공: 165\n추옵: 20/30/40/53/68");
	 }
	 else if(series=="제로")
	 {
		 var name=args[2];
		 if(weaponzero.indexOf(name)!=(-1))
			 msg.channel.send(zero[weaponzero.indexOf(name)]);
		 else
		 {
			 var print="";
			 for(var i=0;i<weaponzero.length;i++)
				 print=print+"\n\n*"+zero[i];
			 printmorezero(client, msg, args,"제로 무기 추옵",print);
		 }
	 }
	 else if(series=="해카세")
	 {
		 msg.channel.send("해방된 카이세리움\n기본공: 400\n추옵: 16/36/59/86/118");
	 }
	 else if(series=="자쿰")
	 {
		 var name=args[2];
		 if(weapon.indexOf(name)!=(-1))
			 msg.channel.send(poisonic[weapon.indexOf(name)]);
		 else
		 {
			 var print="";
			 for(var i=0;i<weapon.length;i++)
				 print=print+"\n\n*"+weapon[i]+"\n"+poisonic[i];
			 printmore(client, msg, args,"자쿰 무기 추옵",print);
		 }
	 }
	 else if(series=="네크로")
	 {
		 var name=args[2];
		 if(weapon.indexOf(name)!=(-1))
			 msg.channel.send(necro[weapon.indexOf(name)]);
		 else
		 {
			 var print="";
			 for(var i=0;i<weapon.length;i++)
				 print=print+"\n\n*"+weapon[i]+"\n"+necro[i];
			 printmore(client, msg, args,"네크로 무기 추옵",print);
		 }
	 }
	 else if(series=="반레온")
	 {
		 var name=args[2];
		 if(weapon.indexOf(name)!=(-1))
			 msg.channel.send(leon[weapon.indexOf(name)]);
		 else
		 {
			 var print="";
			 for(var i=0;i<weapon.length;i++)
				 print=print+"\n\n*"+weapon[i]+"\n"+leon[i];
			 printmore(client, msg, args,"반레온 무기 추옵",print);
		 }
	 }
	 else if(series=="여제")
	 {
		 var name=args[2];
		 if(weapon.indexOf(name)!=(-1))
			 msg.channel.send(cygnus[weapon.indexOf(name)]);
		 else
		 {
			 var print="";
			 for(var i=0;i<weapon.length;i++)
				 print=print+"\n\n*"+weapon[i]+"\n"+cygnus[i];
			 printmore(client, msg, args,"여제 무기 추옵",print);
		 }
	 }
	 else if(series=="파프")
	 {
		 var name=args[2];
		 if(weapon.indexOf(name)!=(-1))
			 msg.channel.send(rootabyss[weapon.indexOf(name)]);
		 else
		 {
			 var print="";
			 for(var i=0;i<weapon.length;i++)
				 print=print+"\n\n*"+weapon[i]+"\n"+rootabyss[i];
			 printmore(client, msg, args,"파프니르 무기 추옵",print);
		 }
	 }
	 else if(series=="앱솔")
	 {
		 var name=args[2];
		 if(weapon.indexOf(name)!=(-1))
			 msg.channel.send(absolabs[weapon.indexOf(name)]);
		 else
		 {
			 var print="";
			 for(var i=0;i<weapon.length;i++)
				 print=print+"\n\n*"+weapon[i]+"\n"+absolabs[i];
			 printmore(client, msg, args,"앱솔랩스 무기 추옵",print);
		 }
	 }
	 else if(series=="아케인")
	 {
		 var name=args[2];
		 if(weapon.indexOf(name)!=(-1))
			 msg.channel.send(arcane[weapon.indexOf(name)]);
		 else
		 {
			 var print="";
			 for(var i=0;i<weapon.length;i++)
				 print=print+"\n\n*"+weapon[i]+"\n"+arcane[i];
			 printmore(client, msg, args,"아케인셰이드 무기 추옵",print);
		 }
	 }
	 else if(series=="제네")
	 {
		 var name=args[2];
		 if(weapon.indexOf(name)!=(-1))
			 msg.channel.send(genesis[weapon.indexOf(name)]);
		 else
		 {
			 var print="";
			 for(var i=0;i<weapon.length;i++)
				 print=print+"\n\n*"+weapon[i]+"\n"+genesis[i];
			 printmore(client, msg, args,"제네시스 무기 추옵",print);
		 }
	 }
	 else
	 {
		 msg.channel.send("추옵 검색 사용법\n\n사용법: !추옵 (장비종류) (무기종류)\n\n장비 종류: 자쿰, 네크로, 반레온, 여제, 파프, 앱솔, 아케인, 제네, 류드, 제로, 해카세\n\n무기 종류: "+weapon.join(", ")+"\n\n제로 무기 종류: 1형~10형\n\n*류드, 해카세의 경우 무기 종류는 입력하지 않습니다.\n*잘못된 값 입력 시 해당 장비 종류 모두를 출력합니다.");
	 }
 }
 
 
 function statanalysis(client, msg, args)
 {
	 var analyze=args;
      for (i=0;i<7;i++) {
 ...
 function armor(client, msg, args)
 {
	 var msgg=msg.content.split("!방무 ")[1];
	 var analyze=msgg.split(' ');
	  ...
	  rate=rate*100;
   if(rate<0) rate=0;
	  msg.channel.send("표기 방무: "+basic+"%,  실 방무: "+real.toFixed(2)+"%\n보스 방어율: "+analyze[0]+"%, 보스에게 딜량: "+rate.toFixed(2)+"%");
...
exports.run = (client, msg, args) => {
		...
		{
			printchuop(client, msg, args);
		}
		else if(args[0]=="!작분석")
		{
			statanalysis(client, msg, args);
		}
		else if(args[0]=="!방무")
		{
			armor(client, msg, args);
		}
		if(msg.content=="@분석")
		{
	let helpMenu = new Menu(msg.channel, msg.author.id, [
            {
                ...
                content: new MessageEmbed({
                    ...
                    description: " ",
                    fields: [
                        { name: "추옵 명령어", value: "!추옵 (장비종류) (무기종류)\n\n장비 종류: 자쿰, 네크로, 반레온, 여제, 파프, 앱솔, 아케인, 제네, 류드, 제로, 해카세\n\n무기 종류: "+weapon.join(", ")+"\n\n제로 무기 종류: 1형~10형\n\n*류드, 해카세의 경우 무기 종류는 입력하지 않습니다.\n*잘못된 값 입력 시 해당 장비 종류 모두를 출력합니다." },
                    ]
                }),
                reactions: {
                    '❌': 'p1',
            ...
            },
			{
                ...
                content: new MessageEmbed({
                    ...
                    description: " ",
                    fields: [
                        { name: "작분석 명령어", value: "!작분석 (총 공격력) (기본공/마) (추옵) (렙제) (스타포스 수치) (작 횟수)\n\n무기의 작을 분석합니다. 없는 항목일 경우 0으로 써주세요!\n예시: !작분석 350 154 62 160 17 8\n\n사용 가능 렙제: 130, 140, 150, 160, 200\n*130제는 20성까지만 있는거 아시죠? ^.^\n\n*주흔작이 아니거나 섞작일 경우 정확한 분석을 하지 못할 수 있습니다." },
                    ]
                }),
                reactions: {
                    '⬅': 'p1',
            ...
            },
            {
                ...
                content: new MessageEmbed({
                    ...
                    description: " ",
                    fields: [
                        { name: "방무계산 명령어", value: "!방무 (보스 방어율) (스텟창 방무) (스킬 방무1) (스킬 방무2) ...\n\n예시: !방무 300 93 15 15 20\n\n*퍼센트 기호는 빼고 숫자만 써주세요!" },
                    ]
                }),
                reactions: {
                    '⬅': 'p2',
